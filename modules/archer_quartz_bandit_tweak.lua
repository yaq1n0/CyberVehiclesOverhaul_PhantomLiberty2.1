ArcherQuartzBanditHandlingTweaksE1 = { 
    description = "Archer Quartz Bandit Handling Tweaks"
}

local VehicleBaseFunctions = require("modules/functions/main_functions")

function ArcherQuartzBanditHandlingTweaksE1.new(FinalDriveMultToSet, MaxTorqueMult, BrakingTorqueMult, BrakingFrictionFactorMult, MaxWheelTurnMult, TurnMaxAddMult, TurnMaxSubMult)
	...
	MaxBrakingTorqueBack=2000*BrakingTorqueMult
	
	ForwardWeightTransfer = 0.45
	airResistanceFactor = 1.6
	TurningRollFactor = 0.92
	TotalMass = 1220
	
	MaxWheelTurnDeg = 50*MaxWheelTurnMult
	TurnMaxAddPerSecond = 90*TurnMaxAddMult
	TurnMaxSubPerSecond = 120*TurnMaxSubMult
	
	FrontFrictionLateralMult=0.965
	FrontFricitionLongitudinal=0.96
	
	RearFrictionLateralMult=1.04
	RearFricitionLongitudinal=1.06
	
	FrontVisualSuspensionDroop = 0.00
	FrontWheelsVerticalOffset = 0.00
	
	RearVisualSuspensionDroop = 0.00
	...
	SwaybarStiffnessFront = 20.5
	SwaybarStiffnessRear = 16.5
	...
	gearsMaxSpeed[4] = 33*FinalDriveMultToSet
	gearsMinRPM[4] = 3500
	gearsMaxRPM[4] = 6400
	...
	gearsMinSpeed[6] = 36*FinalDriveMultToSet
	gearsMaxSpeed[6] = 44.5*FinalDriveMultToSet
	gearsMinRPM[6] = 3900
	gearsMaxRPM[6] = 6000
	
	gearsTorqueMult[7] = 0.27
	gearsMinSpeed[7] = 42*FinalDriveMultToSet
	gearsMaxSpeed[7] = 49.8*FinalDriveMultToSet
	gearsMinRPM[7] = 4000
	gearsMaxRPM[7] = 7100
	
	--I highly suggest not adding _player etc under them, so that you can more simply use them and make it faster for cloning :)
	vehicleName = "standard2_archer_bandit"
	...
	--TweakDB:SetFlat("Vehicle.v_standard2_archer_bandit.vehDriveModelData", TweakDB:GetFlat("Vehicle.v_standard2_archer_quartz_nomad.vehDriveModelData"))
	--TweakDB:SetFlat("Vehicle.v_standard2_archer_bandit.vehEngineData", TweakDB:GetFlat("Vehicle.v_standard2_archer_quartz_nomad.vehEngineData"))
	...
	VehicleBaseFunctions.setVehicleSettings("Vehicle.v_standard2_archer_bandit_player", VehicleEngineData, VehicleDriveModelData, VehicleDataPackage, VehicleWheelDimensionsSetup, false)
	VehicleBaseFunctions.setVehicleSettings("Vehicle.v_standard2_archer_bandit_quest", VehicleEngineData, VehicleDriveModelData, VehicleDataPackage, VehicleWheelDimensionsSetup, false)
...
return ArcherQuartzBanditHandlingTweaksE1
